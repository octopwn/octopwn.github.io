    ...
    this.listDirectory = async function(path) {
        ...
        for (let i = 0; i < res[0].length; i++) {
            ...
            if (entry.get('type') == 'dir') {
                results.push(new FSEntry(this.fstablename, entry.get('name'), fullpath, path, '', true, 0, smbFSdt2js(entry.get("creationtime")), smbFSdt2js(entry.get("lastwritetime")), smbFSdt2js(entry.get("lastaccesstime")), false, this.driver));
            } else {
                results.push(new FSEntry(this.fstablename, entry.get('name'), fullpath, path, '', false, entry.get('size'), smbFSdt2js(entry.get("creationtime")), smbFSdt2js(entry.get("lastwritetime")), smbFSdt2js(entry.get("lastaccesstime")), false, this.driver));
            }
        }
        return results;
    }
