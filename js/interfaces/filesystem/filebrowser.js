...
function FSEntry(fstablename, name, fullpath, root, ext, isdir, size, creationdate, modificationdate, accessdate, readonly, mountpoint) {
    ...
    this.dateColumnFormatter = function(d) {
        try {
            ...
            console.log("Failed to convert: " + d);
            return '';
        }
    }
...
function BrowserFSFileSystem(name) {
    ...
    this.downloadFile = async function(pid, filepath, fileName, fileDownloadProgress, fileDownloadFinished) {
        ...
        fileDownloadFinished(pid, fileName, data, null, true);
    }
...
function FileBrowser(tablename) {
    ...
    this.createFile = async function(fileList) {
        let path = document.getElementById('fbcurrentPath-fileBrowserTable').value;
        let mountpoint = path.substring(0, path.indexOf('/', 1)).substring(1);
        if(mountpoint.startsWith('remote') || mountpoint.startsWith('smb')){
            path = path.substring(2 + mountpoint.length) + '\\';
        }
        else{
    ...
    this.fileDownloadFinished = function(pid, fileName, data, err, overrideSelector = false) {
        
        if (err == null || err == undefined) {
            dltobrowser = document.getElementById('downloadToBrowserSelector').checked;
            if(dltobrowser && !overrideSelector){
                let fs = BrowserFS.BFSRequire('fs');
                let bfsBuffer = BrowserFS.BFSRequire('buffer');
                fileName = fileName.replace('\\', '_').replace('/','_').replace()
                let filepath = '/volatile/' + fileName;
                filedata = bfsBuffer.Buffer.from(data);
        ...
        "fnInitComplete": function() {
            try {
                ...
                <div style="margin-bottom:-35px">
                    <div class="container" id="fileBrowserFunctions">
                        <div class="row">
                            <div class="col-sm">
                        ...
                        <div class="col-sm-8" id="fileBrowserFunctionsDiv">
                            <div id="fileBrowserFunctionsUploadDiv">
                            ...
                            <div id="fileBrowserFunctionsDownloadDiv" title="Checking this will make downloads appear in the browserfs folder instead of directly downloading the files.">
                                <input type="checkbox" id="downloadToBrowserSelector" checked">
                                <label for="downloadToBrowserSelector">Download to browser</label>
                            </div>
                        </div>
                    </div>
                </div>            
